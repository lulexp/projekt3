...
vector<Token> getInputOutput(vector<Token> segment, IOdata* IOdata, string line) {
	...
	if (cmd == "date" || cmd == "time") {
		if (segment.size() != 1) {
			throw new SyntaxException(findIncorrectPosition(segment, 0), line);
		}//TO-DO error;
	...
	if (cmd == "echo") {
			...
			else throw new SyntaxException(findIncorrectPosition(segment, 2), line); //TO-DO error
			return segment;
	}
	...
	if (cmd == "tr") {
			...
			else if (segment.size() == 1) throw new SyntaxException(line.find("tr") + 2, line);
			else
			{
				throw new SyntaxException(findIncorrectPosition(segment, 3), line);
			}
				//TO-DO error;
...
Command* NewParser::parseUnary(vector<Token> segment, string line) {
	...
	if (segment.size() != 2) {
		...
		else throw new SyntaxException(findIncorrectPosition(segment, 1), line);
	}
